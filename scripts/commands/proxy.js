Ape.registerCmd('PROXY_CONNECT', true, function(params, info) {
	...
	socket.onConnect = function() {
		...
		info.user.proxys.set(pipe.getProperty('pubid'), pipe);
		...
		info.user.pipe.sendRaw('PROXY_EVENT', {'event': 'connect', 'chl': this.chl}, {from: this.pipe});
	};
	socket.onRead = function(data) {
		info.user.pipe.sendRaw('PROXY_EVENT', {'event': 'read', 'data': Ape.base64.encode(data)}, {from: this.pipe});
	};
	socket.onDisconnect = function(data) {
		if ($defined(this.pipe)) {
			if (!this.pipe.nouser) { /* User is not available anymore */
				info.user.pipe.sendRaw('PROXY_EVENT', {'event': 'disconnect'}, {from: this.pipe});
				info.user.proxys.erase(this.pipe.getProperty('pubid'));
			}
			/* Destroy the pipe */
			this.pipe.destroy();
		}
